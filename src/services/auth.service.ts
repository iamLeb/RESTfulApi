...
export class AuthService {
  public mailService = new MailServices();

  public async phoneSignUp(userData: CreateUserDto): Promise<User> {
    const findUser: User = await UserModel.findOne({ phone: userData.phone });
    if (findUser) throw new HttpException(409, `This email ${userData.phone} already exists`);
